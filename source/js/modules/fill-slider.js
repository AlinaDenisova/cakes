...
(function () {
  var sliderElement = document.querySelector('.fill'),
      slidesElements = Array.from(sliderElement.querySelectorAll('.fill__item')),
      sliderPreviousButtonElement = document.querySelector('.slider__navigation--fill-previous'),
      sliderNextButtonElement = document.querySelector('.slider__navigation--fill-next'),
      sliderCounterElement = document.querySelector('.form__fill-slider-counter'),
      currentSlideClass = 'fill__item--current',
  ...
  var changeSlide = function (direction) { // Меняет слвйд в зависимости от направления свайпа
    ...
    setTimeout(function () {
      ...
      window.fillSliderIndicator.update(slidesElements, currentSlideIndex);
  ...
  window.fillSlider = {
    add: function() {
      ...
      navigationButtonsUpdate();
      sliderCounterUpdate();
      window.fillSliderIndicator.set(slidesElements);
      window.fillSliderIndicator.update(slidesElements, currentSlideIndex);
    }
  };
})();
